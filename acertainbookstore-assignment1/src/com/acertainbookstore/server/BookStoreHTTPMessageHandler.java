...
public class BookStoreHTTPMessageHandler extends AbstractHandler {
	...
	private void rateBooks(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		Set<BookRating> bookRatingSet = (Set<BookRating>) serializer.get().deserialize(serializedRequestContent);
		BookStoreResponse bookStoreResponse = new BookStoreResponse();

		try {
			myBookStore.rateBooks(bookRatingSet);
		} catch (BookStoreException ex) {
			bookStoreResponse.setException(ex);
		}
	...
	private void getTopRatedBooks(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		Integer numOfBooks = (Integer) serializer.get().deserialize(serializedRequestContent);
		BookStoreResponse bookStoreResponse = new BookStoreResponse();

		try {
			bookStoreResponse.setList(myBookStore.getTopRatedBooks(numOfBooks));
		} catch (BookStoreException ex) {
			bookStoreResponse.setException(ex);
		}
